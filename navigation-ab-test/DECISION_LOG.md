...
- **2026-03-26** — **Secure Prototype Access (Token-Based Authentication)**: Introduced token-based authentication for deployed prototype access. Users must provide a valid access token to enter the application. Token is validated client-side and stored in localStorage for session persistence. Expiration time enforced to require periodic re-authentication. Routing preserves the originally requested path (`redirectTo`) so users return to their intended page after authentication. Invalid or expired tokens redirect to access screen. Rationale: Ensure secure distribution of prototype builds while maintaining seamless navigation experience and minimal disruption to app routing.

- **2026-03-26** — **Desktop Toolbar Visibility (Dismissible Tool Surface)**: Introduced user-controlled visibility for the desktop toolbar. Users can hide the toolbar to reduce persistent chrome and restore it via a recall affordance. Variant A uses a chevron control on the toolbar to collapse it, leaving a minimal bottom-right trigger visible. Variant B uses a top-bar toggle to show/hide the toolbar. This change does not introduce a new surface; it modifies visibility of an existing tool surface. Mobile behavior remains unchanged. Rationale: Improve focus and reduce visual clutter while preserving immediate access to tools.
