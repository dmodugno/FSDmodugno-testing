...
export default function AIChatOverlay({ chat, onClose, onMinimize, onMaximize, onReset, isMinimized, index, drawerOpen, isSplit }) {
  ...
  return (
    <div className={`bg-white flex flex-col transition-all duration-300 w-full h-full ${isSplit ? 'border border-gray-300 rounded-t-lg' : ''}`}>
      {/* Header */}
      <div className="flex items-center justify-between px-4 py-3">
        ...
        <div className="flex items-center space-x-1">
          {onReset && (
            <button
              onClick={onReset}
              className="px-2 py-1 text-sm hover:bg-gray-100 rounded text-gray-700"
              aria-label="New chat"
              title="New/Reset Chat"
            >
              <span>New</span>
      ...
      {/* Messages Area - Gray rounded box */}
      <div className="flex-1 bg-gray-100 rounded-lg overflow-y-auto p-4 m-4 space-y-4">
        {chat.messages.length === 0 ? (
